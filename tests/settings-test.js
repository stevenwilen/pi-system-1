...
(async () => {
  ...
  try {
    console.log('1. what is set, without saying what it is');
    {
      ...
      await post(A, '/settings/calendar', {
        which: 'calendar_ics_url',
        url: `${FEED}/secret-abc123/basic.ics`,
      });
      ...
      check('it says the calendar is set', s.calendar_ics_url.set === true);
      ...
      check('but it is recognisable', /basic\.ics/.test(s.calendar_ics_url.hint),
        s.calendar_ics_url.hint);
      check('the unset one says so', s.calendar_action_ics_url.set === false);
    }

    console.log('\n2. a calendar url that works, is empty, or is dead');
    {
      const full = await (await post(A, '/settings/calendar', {
        which: 'calendar_ics_url', url: `${FEED}/full.ics`,
      })).json();
      ...
      const empty = await (await post(A, '/settings/calendar', {
        which: 'calendar_ics_url', url: `${FEED}/empty.ics`,
      })).json();
      ...
      const dead = await (await post(A, '/settings/calendar', {
        which: 'calendar_ics_url', url: `${FEED}/gone.ics`,
      })).json();
      ...
      const html = await (await post(A, '/settings/calendar', {
        which: 'calendar_ics_url', url: `${FEED}/notacalendar.ics`,
      })).json();
      ...
      const cleared = await (await post(A, '/settings/calendar', {
        which: 'calendar_ics_url', url: null,
      })).json();
    ...
    console.log('\n6. one bad item and NOTHING is written');
    {
      ...
      await post(A, '/settings/calendar', { which: 'calendar_ics_url', url: null });
    ...
    {
      const res = await paste(A, JSON.stringify({
        calendar_ics_url: `${FEED}/gone.ics`,
        calendar_action_ics_url: `${FEED}/empty.ics`,
        items: [],
      }));
      const body = await res.json();

      check('it saves', body.saved === true, JSON.stringify(body).slice(0, 80));

      const dead = (body.checks.feeds || []).find((f) => f.key === 'calendar_ics_url');
      const empty = (body.checks.feeds || []).find((f) => f.key === 'calendar_action_ics_url');

      check('the dead one is named as unreachable', dead && dead.reachable === false,
        JSON.stringify(dead));
      check('the empty one is reachable', empty && empty.reachable === true,
        JSON.stringify(empty));
      check('and reports zero events rather than a failure', empty && empty.events === 0,
        JSON.stringify(empty));
      check('so the two do not read alike', dead.reachable !== empty.reachable);

      // Both stored, because a feed that is down today is not a wrong url.
      const row = await rowOf('a');
      check('both are stored anyway', Boolean(row.calendar_ics_url && row.calendar_action_ics_url));
    }
    ...
    console.log('\n9. the prompt is engine text');
    {
      ...
      check('nothing is interpolated into it', !/\$\{/.test(prompt));
      check('and it names nobody', !/steven|@gmail|b586ea65/i.test(prompt));

      check('it is strictly ordered', /IN ORDER/i.test(prompt));
      check('and refuses to move on', /Do not move to the next step/i.test(prompt));
      ...
      check('then their things', /STEP 3 — THEIR THINGS/.test(prompt));

      check('it verifies by asking for evidence', /paste (?:the number|it|them|the address) back/i.test(prompt),
        'must ask for the value, not "got it?"');
      check('and says not to accept an assurance', /Do not accept "done"/i.test(prompt));
      check('it restates progress in a resumable line',
        /Done: Telegram\. Now: calendars 1 of 2\. Remaining: your things\./.test(prompt));
      check('and says that line is how you resume',
        /resumed|continue from it/i.test(prompt));
      check('it refuses the JSON until everything is verified',
        /Do not produce the final JSON until every step is verified/i.test(prompt));
      check('and names what is missing', /name what is still missing/i.test(prompt));

      // A new person does not know what any of this is for.
      check('it explains the size buckets', /A week of work due in three days/i.test(prompt));
      check('and what staleness is for', /coldest first/i.test(prompt));
      check('and what the two calendars mean', /things to KNOW/.test(prompt) && /things to DO/.test(prompt));
      ...
      check('it warns that the url grants read access',
        /grants read access to that entire calendar/i.test(prompt));
